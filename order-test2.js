...
module.exports = async function() {
  try {
    ...
    const makerAssetAmount = Web3Wrapper.toBaseUnitAmount(new BigNumber(1), 1);
    const takerAssetAmount = Web3Wrapper.toBaseUnitAmount(new BigNumber(1), 1);
    const exchangeAddress = contractWrappers.exchange.address;

    // give allowance
    await fakeToken
      .approve(contractWrappers.contractAddresses.erc20Proxy, new BigNumber(10).pow(256).minus(1), { from:  makerAddress });
